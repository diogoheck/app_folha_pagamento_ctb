...
DADOS_LIDOS_FOLHA = './temp/dados_lidos_folha_prov_13_ant.txt'
DATA_ARQUIVO_FOLHA = './temp/data_arquivo_prov_13.txt'
...
def ler_dados_folha_prov_anter_13(plan_folha):
            ...
            if len(linha_folha[0]) >= 9 and not data:
                if linha_folha[0][0:9].upper() == 'RELATÓRIO':
                    ...
                    with open(DATA_ARQUIVO_FOLHA, 'a', encoding='ansi') as data:
                        print(linha_folha[0][37:44], file=data)

            if len(linha_folha[0]) >= 22:
                if linha_folha[0][0:22] == 'TOTAL CENTRO DE CUSTO:':
                    centro_de_custo = linha_folha[0][23::]
        ...
        # verificar se é um valor valido na coluna do total_saldo
            ...
            if linha_folha:
                if linha_folha[0] == 'Total saldo:' and Rota.centro_custo:
                    ...
                    dicionario_empresas[empresa][centro_de_custo]['PROV13'
                                                                  ] = converte.converte_valor_float(linha_folha[1])
                    dicionario_empresas[empresa][centro_de_custo]['PROVFGTS13'
                                                                  ] = converte.converte_valor_float(linha_folha[2])
                    dicionario_empresas[empresa][centro_de_custo]['PROVINSS13'
...
def ler_folha_provisao_13(plan_prov_13):

    remover_arquivo(DADOS_LIDOS_FOLHA)
    remover_arquivo(DATA_ARQUIVO_FOLHA)

    dic_prov = ler_dados_folha_prov_anter_13(plan_prov_13)
